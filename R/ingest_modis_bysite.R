  ...
  if (prod=="MOD13Q1"){
    ...
    ## QC interpreted according to https://lpdaac.usgs.gov/documents/118/MOD11_User_Guide_V6.pdf
     df <- df %>%
      ...
      if (pixel_quality == "00"){
        ...
        dplyr::mutate(modisvar_filtered = ifelse(data_quality %in% c("00", "01"), modisvar, NA)) %>%

        ## Bits 4-5: Emis Error Flag
        ##   00 = average emissivity error <= 0.01
        ##   01 = average emissivity error <= 0.02
        ##   10 = average emissivity error <= 0.04
        ##   10 =average emissivity error > 0.04
        mutate(emis = substr( qc_bitname, start=4, stop=5 )) %>%
        dplyr::mutate(modisvar_filtered = ifelse(emis %in% c("00", "01"), modisvar, NA)) %>%
        ...
        mutate(lst_error = substr( qc_bitname, start=6, stop=7 )) %>%
        dplyr::mutate(modisvar_filtered = ifelse(lst_error %in% c("01", "10"), modisvar, NA)) %>%
      }
      ## drop it
      dplyr::select(-qc_bitname)
    
   } else if (prod=="MOD21A1D"){  
    ##----------------------------------------
    ## Filter available landsurface temperature data for daily-means
    ##----------------------------------------
    ...
    df <- df %>%
      ...
      if (pixel_quality == "00"){
        ...
        ## Bits 4-5: Emissivity Flag
        ##   00 = >0.02 (Poor performance)
        ##   01 = 0.015 - 0.02 (Marginal performance)
        ##   10 = 0.01 - 0.015 (Good performance)
        ##   1+ = <0.01 (Excellent performance)
        mutate(emissivity = substr( qc_bitname, start=4, stop=5 )) %>%
        dplyr::mutate(modisvar_filtered = ifelse(emissivity %in% c("01", "10", "11"), modisvar, NA)) %>%

        ## Bits 6-7: LST accuracy
        ##   00 = >2 K (Poor performance)
        ##   01 = 01 = 1.5 - 2 K (Marginal performance)
        ##   10 = 1 - 1.5 K (Good performance)
        ##   11 = <1 K (Excellent performance)
        mutate(iterations = substr( qc_bitname, start=6, stop=7 )) %>%
        dplyr::mutate(modisvar_filtered = ifelse(interations %in% c("01", "10", "11"), modisvar, NA)) %>%
