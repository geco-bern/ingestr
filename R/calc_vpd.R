...
#' @export
#' 
calc_vpd <- function( qair=NA, tc=NA, tmin=NA, tmax=NA, patm=NA, elv=NA  ){
  ##-----------------------------------------------------------------------
  ...
  if (is.na(elv) && is.na(patm)) rlang::abort("calc_vpd(): Either patm or elv must be provided.")
  ...
  vpd <- ifelse(!is.na(tmin) && !is.na(tmax),
                mean(
                  calc_vpd_inst( qair=qair, tc=tmin, patm=patm), 
                  calc_vpd_inst( qair=qair, tc=tmax, patm=patm)
                  ),
                calc_vpd_inst( qair=qair, tc=tc, patm=patm)
                )
