...
#' This follows code outlined here: \url{https://gis.stackexchange.com/questions/246476/link-raster-tiff-with-attribute-table-txt-in-qgis-or-r}
#'
#' @param var A character string specifying the variable name (WISE30sec standard)
#' @param df_lonlat A data frame containing columns \code{lon} and \code{lat} specifying points for extraction.
...
#' Defaults to \code{1}.
#' @param dir A character string specifying the path to the local directory containing WISE30sec data.
...
ingest_wise_byvar <- function(var, df_lonlat, layer = 1, dir){
  ...
  df_out <- raster::extract(
    rasta,
    sp::SpatialPoints(df_lonlat %>% dplyr::select(lon, lat)), # , proj4string = rasta@crs
    sp = TRUE
    ) %>%
    as_tibble() %>%
